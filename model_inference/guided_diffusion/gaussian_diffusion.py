            ...
            for t_last, t_cur in time_pairs:
                ...
                if t_cur < t_last:  # reverse
                    with th.no_grad():
                        ...
                        if not os.path.exists('/root/autodl-tmp/ablustion_results/get_process_overview/processs'):
                            os.makedirs('/root/autodl-tmp/ablustion_results/get_process_overview/processs')
                        if (t_last+1)%1 == 0:
                            images = toU8(image_after_step)
                            for i in range(image_after_step.shape[0]):
                                img = images[i]
                                path = os.path.join('/root/autodl-tmp/ablustion_results/get_process_overview/processs',str(i)+'_'+str(t_last)+'.png')
